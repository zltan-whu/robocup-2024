...
cv::Rect get_rect(int roi_w, int roi_h, float bbox[4])
{
    int l, r, t, b;
    float r_w = Yolo::INPUT_W / (roi_w * 1.0);
    float r_h = Yolo::INPUT_H / (roi_h * 1.0);
    if (r_h > r_w)
    {
        l = bbox[0] - bbox[2] / 2.f;
        r = bbox[0] + bbox[2] / 2.f;
        t = bbox[1] - bbox[3] / 2.f - (Yolo::INPUT_H - r_w * roi_h) / 2;
        b = bbox[1] + bbox[3] / 2.f - (Yolo::INPUT_H - r_w * roi_h) / 2;
        l = l / r_w;
        r = r / r_w;
        t = t / r_w;
        b = b / r_w;
    ...
    {
        l = bbox[0] - bbox[2] / 2.f - (Yolo::INPUT_W - r_h * roi_w) / 2;
        r = bbox[0] + bbox[2] / 2.f - (Yolo::INPUT_W - r_h * roi_w) / 2;
        t = bbox[1] - bbox[3] / 2.f;
        b = bbox[1] + bbox[3] / 2.f;
        l = l / r_h;
        r = r / r_h;
    ...
    return cv::Rect(l, t, r - l, b - t);//l,t是指左上坐标 r-l是宽 b-t是高
}
